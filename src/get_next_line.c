/*
** EPITECH PROJECT, 2019
** CPE_getnextline_2018
** File description:
** get_next_line
*/

#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <stdio.h>
#include "get_next_line.h"

char *my_get_strndup(char *str, int n)
{
    char *new_str;
    int len = 0;
    int i = 0;

    while (str[len] != '\0' && (n == -1 || (n != -1 && len < n)))
        len++;
    new_str = malloc(sizeof(char) * (len + 1));
    if (new_str == NULL)
        return (NULL);
    for (; i < len; i++)
        new_str[i] = str[i];
    new_str[len] = '\0';
    return (new_str);
}

char *my_get_strncat(char *dest, char *str, int n)
{
    char *new_str;
    int str_len = 0;
    int dest_len = 0;
    int i = 0;

    while (dest[dest_len] != '\0')
        dest_len++;
    while (str[str_len] != '\0' && (n == -1 || (n != -1 && str_len < n)))
        str_len++;
    new_str = malloc(sizeof(char) * (dest_len + str_len + 1));
    if (new_str == NULL)
        return (NULL);
    for (i = 0; i < dest_len; i++)
        new_str[i] = dest[i];
    for (int j = 0; j < str_len; i++) {
        new_str[i] = str[j];
        j++;
    }
    new_str[i] = '\0';
    return (new_str);
}

static char *read_file(int fd, char *str, char **readed)
{
    char buf[READ_SIZE + 1] = {0};
    int read_value;

    read_value = read(fd, buf, READ_SIZE);
    if (read_value == -1 || str == NULL)
        return (NULL);
    buf[read_value] = '\0';
    for (int i = 0; i < READ_SIZE || buf[i] != '\0'; i++) {
        if (buf[i] == '\n') {
            *readed = my_get_strndup(&buf[i + 1], -1);
            str = my_get_strncat(str, buf, i);
            return (str);
        }
    }
    if (*readed == NULL && read_value == 0 && str[0] == '\0')
        return (NULL);
    if (read_value != READ_SIZE)
        return (my_get_strncat(str, buf, read_value));
    return (read_file(fd, my_get_strncat(str, buf, -1), readed));
}

char *get_next_line(int fd)
{
    static char *readed = NULL;
    char *str;

    if (readed != NULL) {
        str = my_get_strncat(readed, "", -1);
        free(readed);
    }
    else
        str = my_get_strncat("", "", -1);
    readed = NULL;
    if (str == NULL)
        return (NULL);
    for (int i = 0; str[i] != '\0'; i++) {
        if (str[i] == '\n') {
            readed = my_get_strncat(&str[i + 1], "", -1);
            str = my_get_strndup(str, i);
            return (str);
        }
    }
    return (read_file(fd, str, &readed));
}